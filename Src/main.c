...
void send_uart(char *strin)
{
	int len=strlen(strin);
	HAL_UART_Transmit(&huart4,(uint8_t*)strin,len,1000);
}
int main(void)
{
  /* USER CODE BEGIN 1 */
	 ...
	 if (f_mount(&Fatfs, "", 0)==FR_OK)
   {
     HAL_UART_Transmit(&huart4,(uint8_t*)s,strlen(s),HAL_MAX_DELAY);
	  ...
	  fresult = f_open(&fil, "file2.txt", FA_CREATE_ALWAYS | FA_WRITE);
	   strcpy(buffer, "This is File2.txt, written using ...f_write... and it says Hello from Parag\n");
